...
module.exports = (client) => {
  ...
  // Cargar canales guardados desde la base de datos
  const loadSavedChannels = async () => {
    try {
      const data = await Schema.find().exec();
      
      if (data && data.length > 0) {
        const connectPromises = data.map(async (item) => {
          try {
            const channel = await client.channels.fetch(item.Channel);
            if (channel) {
              // Establecer la estación por defecto para este servidor
              client.currentStations.set(channel.guild.id, {
                index: 0,
                name: RADIO_STATIONS[0].name,
                url: RADIO_STATIONS[0].url,
                emoji: RADIO_STATIONS[0].emoji
              });
              return client.radioStart(channel);
            }
          } catch (err) {
            console.error(`Error connecting to channel ${item.Channel}:`, err);
          }
        });
      ...
      console.error("Error loading saved radio channels:", err);
    }
  };
