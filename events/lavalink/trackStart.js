...
module.exports = {
  ...
  async execute(client, player, track) {
    ...
    // Crear barra de progreso inicial
    const progressBar = createProgressBar(0, track.info.duration);

    // Respaldo de avatar y artwork en caso de valores indefinidos
    const avatarURL = client.user?.displayAvatarURL() || 'https://cdn.discordapp.com/embed/avatars/0.png';
    const artworkURL = track.info.artworkUrl || avatarURL;

    // Creación del embed inicial
    const embed = new EmbedBuilder()
      .setColor('#F0E68C')
      .setAuthor({ name: 'Now Playing 🎵', iconURL: avatarURL })
      ...
      .addFields([
        ...
        { name: '🎧 Requested by', value: `${player.requester?.toString() || 'Unknown'}`, inline: true },
      ])
      .setTimestamp()
      .setFooter({
    ...
    const progressInterval = setInterval(() => {
      if (player && !player.paused) {
        const newProgressBar = createProgressBar(player.position, track.info.duration);
    ...
    // Guardar el mensaje en el reproductor
    player.queue.current.userData.nowPlayingMessage = controlMessage;

    // Configurar colector para botones de control
    ...
    collector.on('collect', async (interaction) => {
      try {
        ...
        switch (interaction.customId) {
          ...
          case 'seekforward':
            if (player.position + 10000 > track.info.duration) {
              return interaction.followUp({
